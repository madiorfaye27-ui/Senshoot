// Mirrors lib/utils/pricing.ts from the web app (kept as a separate copy
// rather than a shared import so the existing web app's build/deploy path
// stays untouched — see packages/shared/src/types.ts for the same reasoning).
//
...
// (même règle pour tous les photographes). Le prix de base par photo
// reste fixé par chaque photographe.
...
// This is a client-side preview only — the mobile app must never send a
// computed total to the order API; the server (app/api/orders/route.ts)
// always recomputes the real total from each photo's current price_fcfa.
